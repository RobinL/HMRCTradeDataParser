...
from my_models import Export_EU
...
def raw_eu_export_data_to_database(zipfile, url_info,rawfile):
    ...
    middle_records_specs_dict = get_specs_dict("specs/eu_export_middle.csv")
    ...
    rows = session.query(Export_EU).count()
    logger.debug("there are now {} records in the eu_export table".format(rows))
