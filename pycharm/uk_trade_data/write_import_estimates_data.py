...
from my_models import Import_Estimate
from my_database import session

def raw_import_estimates_data_to_db(zipfile, url_info,rawfile):
    ...
    rows = session.query(Import_Estimate).count()
...
def write_middle_records_to_db(df,rawfile, specs_dict):
    ...
    engine.execute(
        Import_Estimate.__table__.insert(),
        rows_dict
    )